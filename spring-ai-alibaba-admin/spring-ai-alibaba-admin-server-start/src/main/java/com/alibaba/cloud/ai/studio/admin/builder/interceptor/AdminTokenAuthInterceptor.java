...
/**
 * Interceptor for platform administrator authentication on /admin/v1/** endpoints.
 * Validates access tokens and ensures the user has SUPER_ADMIN privileges.
 *
 * @since 1.0.0
 */
@Slf4j
...
public class AdminTokenAuthInterceptor implements HandlerInterceptor {
    ...
    /**
     * Intercepts requests to validate authentication token and verify SUPER_ADMIN privileges.
     * Returns true if authentication is successful and user is a platform administrator.
     */
    @Override
    public boolean preHandle(@NotNull HttpServletRequest request, @NotNull HttpServletResponse response,
        ...
        if (account.getType() != AccountType.SUPER_ADMIN) {
            returnAuthError(start, response, ErrorCode.PERMISSION_DENIED);
            return false;
        }
        ...
        // Initialize tenant context for platform level
    ...
    public void returnAuthError(long start, HttpServletResponse response, ErrorCode errorCode) {
        ...
        try {
            ...
            LogUtils.error("failed to send unauthorized message: {}", result, e);
        }
    }
